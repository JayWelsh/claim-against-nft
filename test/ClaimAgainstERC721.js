...
describe("ClaimAgainstERC721", function () {
  ...
  let claimAgainstERC721WithFeeMinimal;
  ...
  describe("ClaimAgainstERC721WithFeeMinimal", function () {
    beforeEach(async () => {
      // Deploy claimAgainstERC721WithFeeMinimal
      try {
        const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
        blockNumber = await ethers.provider.getBlockNumber();
        block = await ethers.provider.getBlock(blockNumber);
        ...
        incorrectFeeTooHigh = ethers.utils.parseUnits("0.4", "ether"); // 0.4 ETH fee in wei
        incorrectFeeTooLow = ethers.utils.parseUnits("0.1", "ether"); // 0.1 ETH fee in wei
        const ClaimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        payoutAddresses = [firstParty.address, thirdParty.address];
        payoutAddressBasisPoints = [9000, 1000]; // 90%, 10%
        claimAgainstERC721WithFeeMinimal = await ClaimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, payoutAddressBasisPoints);
        await claimAgainstERC721WithFeeMinimal.deployed();
      } catch(e) {
        console.log("Error", e)
      }
    })
    context("deployment", async function () {
      it("Should revert if payoutAddresses length is zero", async function () {
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        await expect(claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, [], payoutAddressBasisPoints)).to.be.revertedWith("ClaimAgainstERC721::constructor: _payoutAddresses must contain at least one entry");
      });
      it("Should revert if payoutAddresses length is not equal to payoutAddressBasisPoints", async function () {
        const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        await expect(claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, [firstParty.address], payoutAddressBasisPoints)).to.be.revertedWith("ClaimAgainstERC721::constructor: each payout address must have a corresponding basis point share");
      });
      it("Should revert if individual payoutAddressBasisPoints value exceeds 10000", async function () {
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        await expect(claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, [10001, 0])).to.be.revertedWith("ClaimAgainstERC721::constructor: _payoutAddressBasisPoints may not contain values of 0 and may not exceed 10000 (100%)");
      });
      it("Should revert if individual payoutAddressBasisPoints value is 0", async function () {
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        await expect(claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, [0, 0])).to.be.revertedWith("ClaimAgainstERC721::constructor: _payoutAddressBasisPoints may not contain values of 0 and may not exceed 10000 (100%)");
      });
      it("Should revert if combined payoutAddressBasisPoints exceeds 10000", async function () {
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        await expect(claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, [9000, 1001])).to.be.revertedWith("ClaimAgainstERC721::constructor: _payoutAddressBasisPoints must add up to 10000 together");
      });
    });
    context("claimAgainstTokenIds(uint256[] memory _tokenIds)", async function () {
      context("revert reasons", async function () {
        it("Should not allow claims to be made before opening time", async function () {
          ...
          for(let account of accounts) {
            tokenId++;
            // Check that claim was successful
            await expect(claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds([tokenId], {value: fee})).to.be.revertedWith("ClaimAgainstERC721::claimAgainstTokenIds: claims have not yet opened");
          }
        })
        it("Should not allow claims to be made after end time", async function () {
          const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
          ...
          for(let account of accounts) {
            tokenId++;
            // Check that claim was successful
            await expect(claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds([tokenId], {value: fee})).to.be.revertedWith("ClaimAgainstERC721::claimAgainstTokenIds: claims have closed");
          }
        })
        it("Should not allow claims to be made with no token IDs provided", async function () {
          const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
          ...
          for(let account of accounts) {
            tokenId++;
            // Check that claim was successful
            await expect(claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds([], {value: fee})).to.be.revertedWith("ClaimAgainstERC721::claimAgainstTokenIds: no token IDs provided");
          }
        })
        it("Should not allow claims to be made without a fee", async function () {
          const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
          ...
          for(let account of accounts) {
            tokenId++;
            // Check that claim was successful
            await expect(claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds([tokenId])).to.be.revertedWith("ClaimAgainstERC721::claimAgainstTokenIds: incorrect claim fee provided");
          }
        })
        it("Should not allow claims to be made with a fee that is too high", async function () {
          const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
          ...
          for(let account of accounts) {
            tokenId++;
            // Check that claim was successful
            await expect(claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds([tokenId], {value: incorrectFeeTooHigh})).to.be.revertedWith("ClaimAgainstERC721::claimAgainstTokenIds: incorrect claim fee provided");
          }
        })
        it("Should not allow claims to be made with a fee that is too low", async function () {
          const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
          ...
          for(let account of accounts) {
            tokenId++;
            // Check that claim was successful
            await expect(claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds([tokenId], {value: incorrectFeeTooLow})).to.be.revertedWith("ClaimAgainstERC721::claimAgainstTokenIds: incorrect claim fee provided");
          }
        })
        it("Should not allow claims to be made against a token ID that has already been claimed against", async function () {
          const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
          ...
          for(let account of accounts) {
            tokenId++;
            await claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds([tokenId], {value: fee});
            // Check that claim was successful
            expect(await claimAgainstERC721WithFeeMinimal.tokenIdToClaimant(tokenId)).to.equal(account.address);
            // Try to claim against token ID again
            await expect(claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds([tokenId], {value: fee})).to.be.revertedWith("ClaimAgainstERC721::claimAgainstTokenIds: token with provided ID has already been claimed against");
          }
        });
        it("Should not allow claims to be made against a token ID that isn't owned by the msg.sender", async function () {
          const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
          ...
          for(let account of accounts) {
            tokenId--;
            // Try to claim against unowned token ID
            await expect(claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds([tokenId], {value: fee})).to.be.revertedWith("ClaimAgainstERC721::claimAgainstTokenIds: msg.sender does not own specified token");
          }
        });
      });
      context("happy path", async function () {
        ...
        it("Should allow each NFT holder to claim against multiple NFTs within one transaction", async function () {
          ...
          for(let account of accounts) {
            ...
            if(tokenId === 100) {
              ...
              await claimAgainstERC721WithFeeMinimal.connect(account).claimAgainstTokenIds(tokenIds, {value: multiTokenClaimFee});
              // Check that claim was successful
              for(let i = 0; i < lastAccountBalance; i++) {
                expect(await claimAgainstERC721WithFeeMinimal.tokenIdToClaimant(tokenId + i)).to.equal(account.address);
              }
            }
          }
        });
    ...
    context("distributeFees()", async function () {
      it("Should distribute funds according to the payoutAddresses & payoutAddressBasisPoints", async function () {
        ...
        // Check balance of claimAgainstERC721WithFeeMinimal
        let balanceOfClaimContractBeforeDistribution = await ethers.provider.getBalance(claimAgainstERC721WithFeeMinimal.address);
        let balanceOfFirstPartyBeforeDistribution = await ethers.provider.getBalance(firstParty.address);
        let balanceOfThirdPartyBeforeDistribution = await ethers.provider.getBalance(thirdParty.address);
        // Run distribution
        await claimAgainstERC721WithFeeMinimal.connect(owner).distributeFees();
        // Check that balance of contract is zero after distribution
        expect(await ethers.provider.getBalance(claimAgainstERC721WithFeeMinimal.address)).to.equal(0);
        for(let [indexString, feeRecipient] of Object.entries(payoutAddresses)) {
          let expectedBasisPoints = payoutAddressBasisPoints[Number(indexString)];
          let expectedFeeValueForRecipient = ethers.BigNumber.from(balanceOfClaimContractBeforeDistribution).mul(expectedBasisPoints).div(10000);
          if(Number(indexString) === 0) {
      ...
      it("Should revert if a fee cut delivery is unsuccessful", async function () {
        const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        const claimAgainstERC721WithFeeMinimalDeliveryFailure = await claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, [firstParty.address, mockRejectETH.address], payoutAddressBasisPoints);
        await claimAgainstERC721WithFeeMinimalDeliveryFailure.deployed();
        // Increase to opening time
        await hre.network.provider.request({
          method: "evm_setNextBlockTimestamp",
          params: [Number(startTime)],
        ...
        for(let account of accounts) {
          tokenId++;
          await claimAgainstERC721WithFeeMinimalDeliveryFailure.connect(account).claimAgainstTokenIds([tokenId], {value: fee});
          // Check that claim was successful
          expect(await claimAgainstERC721WithFeeMinimalDeliveryFailure.tokenIdToClaimant(tokenId)).to.equal(account.address);
        }
        // Run distribution
        await expect(claimAgainstERC721WithFeeMinimalDeliveryFailure.connect(owner).distributeFees()).to.be.revertedWith("ClaimAgainstERC721::distributeFees: Fee cut delivery unsuccessful");
      });
    });
    context("updateFeePayoutScheme", async function () {
      it("Should revert if payoutAddresses length is zero", async function () {
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        const claimAgainstERC721WithFeeMinimalUpdatePayoutScheme = await claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, payoutAddressBasisPoints);
        await claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.deployed();
        await expect(claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.updateFeePayoutScheme([], payoutAddressBasisPoints)).to.be.revertedWith("laimAgainstERC721::updateFeePayoutScheme: _payoutAddresses must contain at least one entry");
      });
      it("Should revert if payoutAddresses length is not equal to payoutAddressBasisPoints", async function () {
        const [owner, firstParty, thirdParty, ...accounts] = await ethers.getSigners();
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        const claimAgainstERC721WithFeeMinimalUpdatePayoutScheme = await claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, payoutAddressBasisPoints);
        await claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.deployed();
        await expect(claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.updateFeePayoutScheme([firstParty.address], payoutAddressBasisPoints)).to.be.revertedWith("ClaimAgainstERC721::updateFeePayoutScheme: each payout address must have a corresponding basis point share");
      });
      it("Should revert if individual payoutAddressBasisPoints value exceeds 10000", async function () {
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        const claimAgainstERC721WithFeeMinimalUpdatePayoutScheme = await claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, payoutAddressBasisPoints);
        await claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.deployed();
        await expect(claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.updateFeePayoutScheme(payoutAddresses, [10001, 0])).to.be.revertedWith("ClaimAgainstERC721::updateFeePayoutScheme: _payoutAddressBasisPoints may not contain values of 0 and may not exceed 10000 (100%)");
      });
      it("Should revert if individual payoutAddressBasisPoints value is 0", async function () {
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        const claimAgainstERC721WithFeeMinimalUpdatePayoutScheme = await claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, payoutAddressBasisPoints);
        await claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.deployed();
        await expect(claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.updateFeePayoutScheme(payoutAddresses, [0, 0])).to.be.revertedWith("ClaimAgainstERC721::updateFeePayoutScheme: _payoutAddressBasisPoints may not contain values of 0 and may not exceed 10000 (100%)");
      });
      it("Should revert if combined payoutAddressBasisPoints exceeds 10000", async function () {
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        const claimAgainstERC721WithFeeMinimalUpdatePayoutScheme = await claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, payoutAddressBasisPoints);
        await claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.deployed();
        await expect(claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.updateFeePayoutScheme(payoutAddresses, [9000, 1001])).to.be.revertedWith("ClaimAgainstERC721::updateFeePayoutScheme: _payoutAddressBasisPoints must add up to 10000 together");
      });
      it("Should update payout scheme when provided valid values", async function () {
        const [owner, firstParty, thirdParty, newPayoutAddress1, newPayoutAddress2, ...accounts] = await ethers.getSigners();
        const claimAgainstERC721WithFeeMinimal = await ethers.getContractFactory("ClaimAgainstERC721WithFeeMinimal");
        const claimAgainstERC721WithFeeMinimalUpdatePayoutScheme = await claimAgainstERC721WithFeeMinimal.deploy(mockERC721.address, startTime, endTime, fee, payoutAddresses, payoutAddressBasisPoints);
        await claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.deployed();
        await expect(claimAgainstERC721WithFeeMinimalUpdatePayoutScheme.updateFeePayoutScheme([owner.address, firstParty.address, newPayoutAddress1.address, newPayoutAddress2.address], [8000, 1000, 500, 500])).to.emit(claimAgainstERC721WithFeeMinimalUpdatePayoutScheme, "UpdatedPayoutScheme");
      });
    });
  });
